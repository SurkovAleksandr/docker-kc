...
def get_record():
    try:
        ...
        cur = conn.cursor()
        cur.execute("SELECT id, text, status FROM example.app_table;")
...
def save_record():
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO example.app_table (id, text, status) VALUES (%s, %s, %s)",
...
def delete_record():
    ...
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("DELETE FROM example.app_table WHERE id = %s", (data["id"],))
        conn.commit()
        cur.close()
        conn.close()
    except psycopg2.DatabaseError as e:
